...
/**
 * Unit Testing: intended for local network (localhost/hardhat) - dev networks
 * - Deploys contract/dependant contracts before testing using fixtures, marking a distinction between deploy and test tasks
 * - The deployment scripts at /deploy are responsible for detecting which network they are being deployed to and using mocks if dev networks
 * - Skips the test altogether if specifying a test network
 */
 ...
 */
// Only run in dev, not test networks
  ...
  : describe('ERC20 Unit Tests', () => {
      ...
      describe('Deployment', () => {
        ...
        it('emits Transfer event on assigning the total supply to the owner', async () => {
          // How to test events at deployment time??
        });
      });
